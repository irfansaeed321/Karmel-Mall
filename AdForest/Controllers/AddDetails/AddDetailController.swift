    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if section == 0 {
            ...
            cell.btnMakeFeature = { () in
                let param: [String: Any] = ["ad_id": objData.adDetail.adId ?? 0]
...
//                cell.lblHtmlText.attributedText = NSMutableAttributedString(string: (attrStr?.string)!, attributes: strokeTextAttributes)
            ...
            do {
               try cell.imgThumb.setGifImage(UIImage(gifName: "thumb"), manager: gifManager, loopCount: -1)
               try cell.imgHeart.setGifImage(UIImage(gifName: "heart"), manager: gifManager, loopCount: -1)
               try cell.imgWow.setGifImage(UIImage(gifName: "wow"), manager: gifManager, loopCount: -1)
               try cell.imgAndry.setGifImage(UIImage(gifName: "sad"), manager: gifManager, loopCount: -1)
            } catch let error {
